...
// ============================================================
// /kerjasama/[slug] — form pengajuan kerjasama PUBLIK (dinamis)
// ============================================================
...
interface Program { judul: string; kategori: string; deskripsi: string | null; fotoUrl?: string | null; fields: Field[] }

export default function KerjasamaApplyPage() {
  ...
  useEffect(() => {
    fetch(`/api/kerjasama-form/${slug}`).then(r => r.ok ? r.json() : Promise.reject()).then(setProgram).catch(() => setNotFound(true))
  }, [slug])
  ...
  async function submit() {
    ...
    setSubmitting(true)
    const res = await fetch(`/api/kerjasama-form/${slug}`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ nama: nama.trim(), perusahaan: perusahaan.trim(), whatsapp: whatsapp.trim(), email: email.trim(), jawaban }),
    })
    setSubmitting(false)
  ...
  if (notFound) return <Shell><p className="text-gray-400 text-sm text-center">Program kerjasama tidak ditemukan atau sudah ditutup.</p></Shell>
  if (!program) return <Shell><div className="flex justify-center py-16 text-gray-400"><Loader2 className="w-6 h-6 animate-spin" /></div></Shell>
  if (done) return (
    <Shell>
      <div className="text-center space-y-3 py-10">
        ...
        <h2 className="font-bold text-lg">Pengajuan Terkirim!</h2>
        <p className="text-sm text-gray-500">Terima kasih, {nama.split(' ')[0]}. Tim kami akan meninjau dan menghubungimu segera.</p>
      </div>
    </Shell>
  )
  ...
  return (
    <Shell>
      {program.fotoUrl && (
        ...
        <img src={program.fotoUrl} alt={program.judul} className="w-full rounded-xl mb-6 border border-gray-100" />
      )}
      <div className="mb-6">
        <p className="text-[11px] text-gray-400 uppercase tracking-widest mb-1">Pengajuan Kerjasama</p>
        <h1 className="text-2xl font-bold">{program.judul}</h1>
        <p className="text-sm text-blue-600 font-semibold">{program.kategori}</p>
        {program.deskripsi && <p className="text-sm text-gray-600 mt-3 whitespace-pre-line">{program.deskripsi}</p>}
      </div>
      {error && <div className="mb-4 bg-red-50 border border-red-200 text-red-600 text-sm rounded-lg px-4 py-3">{error}</div>}
      <div className="space-y-4">
        <Input label="Nama Lengkap / PIC *" value={nama} onChange={setNama} />
        <Input label="Nama Perusahaan / Brand" value={perusahaan} onChange={setPerusahaan} />
          ...
          <div key={f.key}>
            ...
            {f.type === 'paragraf' ? (
              ...
              <input type={f.type === 'angka' ? 'number' : f.type === 'link' ? 'url' : 'text'}
                ...
                placeholder={f.type === 'link' ? 'https://… (mis. company profile / proposal)' : ''}
