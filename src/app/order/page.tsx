...
export default function OrderPublikPage() {
  ...
  useEffect(() => {
    fetch('/api/order-publik').then(r => r.json()).then(d => {
      if (Array.isArray(d.branches)) { setBranches(d.branches); if (d.branches[0]) setBranchId(d.branches[0].id) }
      if (Array.isArray(d.pakets)) setPakets(d.pakets)
      setLoading(false)
    }).catch(() => { setError('Gagal memuat'); setLoading(false) })
  }, [])

  // Paket untuk studio terpilih (paket tanpa branchId = berlaku semua)
  const visiblePakets = useMemo(
    () => pakets.filter(p => !p.branchId || p.branchId === branchId),
    [pakets, branchId]
  )
          ...
          {done ? (
            ...
            <>
              <div className="mb-6">
                ...
                <h1 className="text-2xl font-bold">Pesan Layanan</h1>
                <p className="text-sm text-gray-500 mt-1">Cetak foto, pasfoto, photobox, dan lainnya — tanpa antri, bayar di studio.</p>
              </div>
              {error && <div className="mb-4 bg-red-50 border border-red-200 text-red-600 text-sm rounded-lg px-4 py-3">{error}</div>}

              <div className="space-y-4">
                <div>
                  <label className="block text-xs font-semibold text-gray-600 mb-1.5">Studio</label>
                  <select value={branchId} onChange={e => setBranchId(e.target.value)} className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm bg-white">
                    {branches.filter(b => b.slug !== 'booth').map(b => <option key={b.id} value={b.id}>{b.nama}</option>)}
                  </select>
                </div>
